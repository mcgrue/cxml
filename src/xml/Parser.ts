...
export class Parser {
  attach<CustomHandler extends HandlerInstance>(handler: {
    ...
    var realProto: any = realHandler.prototype as CustomHandler;
  ...
  _parse<Output extends HandlerInstance>(
    ...
    var xmlSpace = this.context.registerNamespace(
      ...
      for (var key of Object.keys(attrTbl)) {
        if (key.substr(0, 5) == 'xmlns') {
          ...
          if (nsParts) {
            state.addNamespace(
              nsParts[2] || '',
              this.context.registerNamespace(attrTbl[key]),
            );
          }
        }
      }
    ...
    xml.on('closetag', function (name: string) {
      ...
      if (member == undefined) {
        throw Error(
          `'${this.name}' is not a known member tag of the xml object you are trying to parse.  Check the capitalization?`,
        );
      }
